...
public class Preview implements SurfaceHolder.Callback, TextureView.SurfaceTextureListener {
    ...
    private void initCameraParameters() throws CameraControllerException {
...

        {
            ...
            //Ajout danem pour avoir uniquement des photo de 1Mpixel max 54345687486514546
            List<CameraController.Size> photo_sizes_limited = new ArrayList<CameraController.Size>();
            for(int i=0;i<photo_sizes.size();i++) {
                CameraController.Size size = photo_sizes.get(i);
                if(  size.width*size.height < 1000000 ) {
                    photo_sizes_limited.add(size) ;
                }
            }
            //Si on n'a rien repris, on prends la première resol
            if (photo_sizes_limited.size() == 0 )
                photo_sizes_limited.add(photo_sizes.get(0)) ;

            this.photo_sizes = photo_sizes_limited ;
            //Fin modif danem
